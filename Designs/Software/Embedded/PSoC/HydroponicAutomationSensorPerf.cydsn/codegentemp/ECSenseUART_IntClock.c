/*******************************************************************************
* File Name: ECSenseUART_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: ECSenseUART_IntClock_Start
********************************************************************************
*
* Summary:
...
void ECSenseUART_IntClock_Start(void) 
{
    ...
    ECSenseUART_IntClock_CLKEN |= ECSenseUART_IntClock_CLKEN_MASK;
	ECSenseUART_IntClock_CLKSTBY |= ECSenseUART_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ECSenseUART_IntClock_Stop
********************************************************************************
*
* Summary:
...
void ECSenseUART_IntClock_Stop(void) 
{
    ...
    ECSenseUART_IntClock_CLKEN &= (uint8)(~ECSenseUART_IntClock_CLKEN_MASK);
	ECSenseUART_IntClock_CLKSTBY &= (uint8)(~ECSenseUART_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ECSenseUART_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ECSenseUART_IntClock_CLKEN & ECSenseUART_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ECSenseUART_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ECSenseUART_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ECSenseUART_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ECSenseUART_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ECSenseUART_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ECSenseUART_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ECSenseUART_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ECSenseUART_IntClock_GetSourceRegister();
    uint16 oldDivider = ECSenseUART_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ECSenseUART_IntClock_CLKEN & ECSenseUART_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ECSenseUART_IntClock_DIV_PTR, clkDivider);
                ECSenseUART_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ECSenseUART_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ECSenseUART_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ECSenseUART_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ECSenseUART_IntClock_DIV_PTR, clkDivider);
				ECSenseUART_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ECSenseUART_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ECSenseUART_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: ECSenseUART_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ECSenseUART_IntClock_MOD_SRC |= modeBitMask & (uint8)ECSenseUART_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: ECSenseUART_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ECSenseUART_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ECSenseUART_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: ECSenseUART_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ECSenseUART_IntClock_MOD_SRC & (uint8)(ECSenseUART_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: ECSenseUART_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ECSenseUART_IntClock_GetDividerRegister();
    uint8 oldSrc = ECSenseUART_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ECSenseUART_IntClock_MOD_SRC |= CYCLK_SSS;
        ECSenseUART_IntClock_MOD_SRC =
            (ECSenseUART_IntClock_MOD_SRC & (uint8)(~ECSenseUART_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ECSenseUART_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ECSenseUART_IntClock_MOD_SRC & ECSenseUART_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ECSenseUART_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ECSenseUART_IntClock_PHASE = clkPhase & ECSenseUART_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ECSenseUART_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ECSenseUART_IntClock_PHASE & ECSenseUART_IntClock_PHASE_MASK;
}
