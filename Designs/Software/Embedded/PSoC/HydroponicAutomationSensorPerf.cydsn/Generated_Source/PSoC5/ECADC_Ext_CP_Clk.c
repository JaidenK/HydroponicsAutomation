...
/*******************************************************************************
* Function Name: ECADC_Ext_CP_Clk_Start
********************************************************************************
*
* Summary:
...
void ECADC_Ext_CP_Clk_Start(void) 
{
    ...
    ECADC_Ext_CP_Clk_CLKEN |= ECADC_Ext_CP_Clk_CLKEN_MASK;
	ECADC_Ext_CP_Clk_CLKSTBY |= ECADC_Ext_CP_Clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ECADC_Ext_CP_Clk_Stop
********************************************************************************
*
* Summary:
...
void ECADC_Ext_CP_Clk_Stop(void) 
{
    ...
    ECADC_Ext_CP_Clk_CLKEN &= (uint8)(~ECADC_Ext_CP_Clk_CLKEN_MASK);
	ECADC_Ext_CP_Clk_CLKSTBY &= (uint8)(~ECADC_Ext_CP_Clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ECADC_Ext_CP_Clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ECADC_Ext_CP_Clk_CLKEN & ECADC_Ext_CP_Clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ECADC_Ext_CP_Clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ECADC_Ext_CP_Clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ECADC_Ext_CP_Clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ECADC_Ext_CP_Clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ECADC_Ext_CP_Clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ECADC_Ext_CP_Clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ECADC_Ext_CP_Clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ECADC_Ext_CP_Clk_GetSourceRegister();
    uint16 oldDivider = ECADC_Ext_CP_Clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ECADC_Ext_CP_Clk_CLKEN & ECADC_Ext_CP_Clk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ECADC_Ext_CP_Clk_DIV_PTR, clkDivider);
                ECADC_Ext_CP_Clk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ECADC_Ext_CP_Clk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ECADC_Ext_CP_Clk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ECADC_Ext_CP_Clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ECADC_Ext_CP_Clk_DIV_PTR, clkDivider);
				ECADC_Ext_CP_Clk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ECADC_Ext_CP_Clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ECADC_Ext_CP_Clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: ECADC_Ext_CP_Clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ECADC_Ext_CP_Clk_MOD_SRC |= modeBitMask & (uint8)ECADC_Ext_CP_Clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: ECADC_Ext_CP_Clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ECADC_Ext_CP_Clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ECADC_Ext_CP_Clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: ECADC_Ext_CP_Clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ECADC_Ext_CP_Clk_MOD_SRC & (uint8)(ECADC_Ext_CP_Clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: ECADC_Ext_CP_Clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ECADC_Ext_CP_Clk_GetDividerRegister();
    uint8 oldSrc = ECADC_Ext_CP_Clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ECADC_Ext_CP_Clk_MOD_SRC |= CYCLK_SSS;
        ECADC_Ext_CP_Clk_MOD_SRC =
            (ECADC_Ext_CP_Clk_MOD_SRC & (uint8)(~ECADC_Ext_CP_Clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ECADC_Ext_CP_Clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ECADC_Ext_CP_Clk_MOD_SRC & ECADC_Ext_CP_Clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ECADC_Ext_CP_Clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ECADC_Ext_CP_Clk_PHASE = clkPhase & ECADC_Ext_CP_Clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ECADC_Ext_CP_Clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ECADC_Ext_CP_Clk_PHASE & ECADC_Ext_CP_Clk_PHASE_MASK;
}
