/*******************************************************************************
* File Name: pHDelSigADC_theACLK.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: pHDelSigADC_theACLK_Start
********************************************************************************
*
* Summary:
...
void pHDelSigADC_theACLK_Start(void) 
{
    ...
    pHDelSigADC_theACLK_CLKEN |= pHDelSigADC_theACLK_CLKEN_MASK;
	pHDelSigADC_theACLK_CLKSTBY |= pHDelSigADC_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: pHDelSigADC_theACLK_Stop
********************************************************************************
*
* Summary:
...
void pHDelSigADC_theACLK_Stop(void) 
{
    ...
    pHDelSigADC_theACLK_CLKEN &= (uint8)(~pHDelSigADC_theACLK_CLKEN_MASK);
	pHDelSigADC_theACLK_CLKSTBY &= (uint8)(~pHDelSigADC_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: pHDelSigADC_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((pHDelSigADC_theACLK_CLKEN & pHDelSigADC_theACLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = pHDelSigADC_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = pHDelSigADC_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* pHDelSigADC_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(pHDelSigADC_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(pHDelSigADC_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: pHDelSigADC_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: pHDelSigADC_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = pHDelSigADC_theACLK_GetSourceRegister();
    uint16 oldDivider = pHDelSigADC_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = pHDelSigADC_theACLK_CLKEN & pHDelSigADC_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(pHDelSigADC_theACLK_DIV_PTR, clkDivider);
                pHDelSigADC_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                pHDelSigADC_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(pHDelSigADC_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((pHDelSigADC_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(pHDelSigADC_theACLK_DIV_PTR, clkDivider);
				pHDelSigADC_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: pHDelSigADC_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(pHDelSigADC_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: pHDelSigADC_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    pHDelSigADC_theACLK_MOD_SRC |= modeBitMask & (uint8)pHDelSigADC_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: pHDelSigADC_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    pHDelSigADC_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(pHDelSigADC_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: pHDelSigADC_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return pHDelSigADC_theACLK_MOD_SRC & (uint8)(pHDelSigADC_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: pHDelSigADC_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = pHDelSigADC_theACLK_GetDividerRegister();
    uint8 oldSrc = pHDelSigADC_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        pHDelSigADC_theACLK_MOD_SRC |= CYCLK_SSS;
        pHDelSigADC_theACLK_MOD_SRC =
            (pHDelSigADC_theACLK_MOD_SRC & (uint8)(~pHDelSigADC_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: pHDelSigADC_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return pHDelSigADC_theACLK_MOD_SRC & pHDelSigADC_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: pHDelSigADC_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    pHDelSigADC_theACLK_PHASE = clkPhase & pHDelSigADC_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: pHDelSigADC_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return pHDelSigADC_theACLK_PHASE & pHDelSigADC_theACLK_PHASE_MASK;
}
