/*******************************************************************************
* File Name: SensorDataCom_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: SensorDataCom_IntClock_Start
********************************************************************************
*
* Summary:
...
void SensorDataCom_IntClock_Start(void) 
{
    ...
    SensorDataCom_IntClock_CLKEN |= SensorDataCom_IntClock_CLKEN_MASK;
	SensorDataCom_IntClock_CLKSTBY |= SensorDataCom_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: SensorDataCom_IntClock_Stop
********************************************************************************
*
* Summary:
...
void SensorDataCom_IntClock_Stop(void) 
{
    ...
    SensorDataCom_IntClock_CLKEN &= (uint8)(~SensorDataCom_IntClock_CLKEN_MASK);
	SensorDataCom_IntClock_CLKSTBY &= (uint8)(~SensorDataCom_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SensorDataCom_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SensorDataCom_IntClock_CLKEN & SensorDataCom_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SensorDataCom_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SensorDataCom_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SensorDataCom_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SensorDataCom_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SensorDataCom_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SensorDataCom_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SensorDataCom_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SensorDataCom_IntClock_GetSourceRegister();
    uint16 oldDivider = SensorDataCom_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SensorDataCom_IntClock_CLKEN & SensorDataCom_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(SensorDataCom_IntClock_DIV_PTR, clkDivider);
                SensorDataCom_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                SensorDataCom_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(SensorDataCom_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SensorDataCom_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(SensorDataCom_IntClock_DIV_PTR, clkDivider);
				SensorDataCom_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: SensorDataCom_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(SensorDataCom_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: SensorDataCom_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SensorDataCom_IntClock_MOD_SRC |= modeBitMask & (uint8)SensorDataCom_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: SensorDataCom_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SensorDataCom_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SensorDataCom_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: SensorDataCom_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SensorDataCom_IntClock_MOD_SRC & (uint8)(SensorDataCom_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: SensorDataCom_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SensorDataCom_IntClock_GetDividerRegister();
    uint8 oldSrc = SensorDataCom_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SensorDataCom_IntClock_MOD_SRC |= CYCLK_SSS;
        SensorDataCom_IntClock_MOD_SRC =
            (SensorDataCom_IntClock_MOD_SRC & (uint8)(~SensorDataCom_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SensorDataCom_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SensorDataCom_IntClock_MOD_SRC & SensorDataCom_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SensorDataCom_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SensorDataCom_IntClock_PHASE = clkPhase & SensorDataCom_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SensorDataCom_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SensorDataCom_IntClock_PHASE & SensorDataCom_IntClock_PHASE_MASK;
}
