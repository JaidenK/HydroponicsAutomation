/*******************************************************************************
* File Name: pHDelSigADC_Ext_CP_Clk.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: pHDelSigADC_Ext_CP_Clk_Start
********************************************************************************
*
* Summary:
...
void pHDelSigADC_Ext_CP_Clk_Start(void) 
{
    ...
    pHDelSigADC_Ext_CP_Clk_CLKEN |= pHDelSigADC_Ext_CP_Clk_CLKEN_MASK;
	pHDelSigADC_Ext_CP_Clk_CLKSTBY |= pHDelSigADC_Ext_CP_Clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: pHDelSigADC_Ext_CP_Clk_Stop
********************************************************************************
*
* Summary:
...
void pHDelSigADC_Ext_CP_Clk_Stop(void) 
{
    ...
    pHDelSigADC_Ext_CP_Clk_CLKEN &= (uint8)(~pHDelSigADC_Ext_CP_Clk_CLKEN_MASK);
	pHDelSigADC_Ext_CP_Clk_CLKSTBY &= (uint8)(~pHDelSigADC_Ext_CP_Clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: pHDelSigADC_Ext_CP_Clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((pHDelSigADC_Ext_CP_Clk_CLKEN & pHDelSigADC_Ext_CP_Clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = pHDelSigADC_Ext_CP_Clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = pHDelSigADC_Ext_CP_Clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* pHDelSigADC_Ext_CP_Clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(pHDelSigADC_Ext_CP_Clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(pHDelSigADC_Ext_CP_Clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: pHDelSigADC_Ext_CP_Clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: pHDelSigADC_Ext_CP_Clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = pHDelSigADC_Ext_CP_Clk_GetSourceRegister();
    uint16 oldDivider = pHDelSigADC_Ext_CP_Clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = pHDelSigADC_Ext_CP_Clk_CLKEN & pHDelSigADC_Ext_CP_Clk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(pHDelSigADC_Ext_CP_Clk_DIV_PTR, clkDivider);
                pHDelSigADC_Ext_CP_Clk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                pHDelSigADC_Ext_CP_Clk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(pHDelSigADC_Ext_CP_Clk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((pHDelSigADC_Ext_CP_Clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(pHDelSigADC_Ext_CP_Clk_DIV_PTR, clkDivider);
				pHDelSigADC_Ext_CP_Clk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: pHDelSigADC_Ext_CP_Clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(pHDelSigADC_Ext_CP_Clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: pHDelSigADC_Ext_CP_Clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    pHDelSigADC_Ext_CP_Clk_MOD_SRC |= modeBitMask & (uint8)pHDelSigADC_Ext_CP_Clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: pHDelSigADC_Ext_CP_Clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    pHDelSigADC_Ext_CP_Clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(pHDelSigADC_Ext_CP_Clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: pHDelSigADC_Ext_CP_Clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return pHDelSigADC_Ext_CP_Clk_MOD_SRC & (uint8)(pHDelSigADC_Ext_CP_Clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: pHDelSigADC_Ext_CP_Clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = pHDelSigADC_Ext_CP_Clk_GetDividerRegister();
    uint8 oldSrc = pHDelSigADC_Ext_CP_Clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        pHDelSigADC_Ext_CP_Clk_MOD_SRC |= CYCLK_SSS;
        pHDelSigADC_Ext_CP_Clk_MOD_SRC =
            (pHDelSigADC_Ext_CP_Clk_MOD_SRC & (uint8)(~pHDelSigADC_Ext_CP_Clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: pHDelSigADC_Ext_CP_Clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return pHDelSigADC_Ext_CP_Clk_MOD_SRC & pHDelSigADC_Ext_CP_Clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: pHDelSigADC_Ext_CP_Clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    pHDelSigADC_Ext_CP_Clk_PHASE = clkPhase & pHDelSigADC_Ext_CP_Clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: pHDelSigADC_Ext_CP_Clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return pHDelSigADC_Ext_CP_Clk_PHASE & pHDelSigADC_Ext_CP_Clk_PHASE_MASK;
}
